...
use bevy::{
    prelude::*,
    window::PrimaryWindow, asset::ChangeWatcher, app::AppExit,
};
use rand::prelude::*;
...
pub fn spawn_enemies(
    ...
    for _ in 0..NUMBER_OF_ENEMIES {
        let randow_x = random::<f32>() * window.width();
        let randow_y = random::<f32>() * window.height();

        commands.spawn(
            (
                SpriteBundle {
                    transform: Transform::from_xyz(randow_x, randow_y, 0.0),
...
pub fn confine_player_movement(
    ...
    if let Ok(mut player_transform) = player_query.get_single_mut() {
        ...
        let mut traslation = player_transform.translation;

        if traslation.x < x_min {
            traslation.x = x_min;
        } else if traslation.x > x_max {
            traslation.x = x_max;
        }
        if traslation.y < y_min {
            traslation.y = y_min;
        } else if traslation.y > y_max {
            traslation.y = y_max;
        }

        player_transform.translation = traslation;
    }
}
